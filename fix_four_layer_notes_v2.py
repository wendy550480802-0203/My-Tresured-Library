...
"""
修复四层笔记匹配脚本 v2
- 正确匹配：文件名包含节点标题（而不是双向子串匹配）
- 只有书籍节点才可能有四层笔记，概念节点不应该匹配
"""
...
def find_matching_notes(title, notes_index):
    """查找匹配的四层笔记 - 严格匹配：只匹配文件名包含节点标题的情况"""
    if not title:
        return None
    ...
    # 3. 节点标题包含书名（处理节点标题更长的情况）
    for book_title, (filename, content) in notes_index.items():
        if book_title in title_clean:
            return content
    ...
    if title_no_punct:
        for book_title, (filename, content) in notes_index.items():
            ...
            filename_no_punct = re.sub(r'[^\w\s]', '', filename)
            if title_no_punct in filename_no_punct or book_no_punct in title_no_punct:
                return content
