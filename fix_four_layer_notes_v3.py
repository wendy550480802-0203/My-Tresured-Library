...
"""
修复四层笔记匹配脚本 v3 - 简化版
- 只给type=book的节点添加四层笔记
- 严格匹配：节点标题精确匹配书名（从文件名提取）
"""
...
def process_html_file(html_file, notes_index):
    ...
    try:
        ...
        if updated == 0:
            print(f"  ○ 无匹配: {html_file.name} (书籍节点数: {sum(1 for n in nodes if n.get('type') == 'book')})")
            return 0
        ...
        print(f"  ✓ 更新了 {updated}/{sum(1 for n in nodes if n.get('type') == 'book')} 个书籍节点: {html_file.name}")
        return updated
